...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, num_group=16):
        super(BasicBlock, self).__init__()
        self.conv1 = conv3x3(inplanes, planes*2, stride)
        self.bn1 = nn.BatchNorm2d(planes*2)
        self.relu = nn.ReLU(inplace=True)
        # self.conv2 = conv3x3(planes*2, planes*2, groups=num_group)
        self.conv2 = nn.Conv2d(planes*2,planes*2,kernel_size=3,stride=1,padding=1,bias=False,groups=num_group)
...
class ResNeXt(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, num_group=16):
        self.inplanes = 64
